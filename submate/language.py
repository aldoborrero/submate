"""Language code enum with ISO 639-1/2 support.

Ported from the original subgen implementation for comprehensive
language code handling in skip logic and subtitle naming.
"""
...
class LanguageCode(Enum):
    """Comprehensive language code enum with ISO 639-1, ISO 639-2/T, ISO 639-2/B support.

    Each value is a tuple of (iso_639_1, iso_639_2_t, iso_639_2_b, name_en, name_native).
    """
    ...
    NONE = (None, None, None, None, None)  # For no/unknown language

    def __init__(
        self,
        iso_639_1: str | None,
        iso_639_2_t: str | None,
        iso_639_2_b: str | None,
        name_en: str | None,
    ...
    @staticmethod
    def from_iso_639_1(code: str | None) -> "LanguageCode":
        """Get LanguageCode from ISO 639-1 code (e.g., 'en')."""
        if not code:
    ...
    @staticmethod
    def from_iso_639_2(code: str | None) -> "LanguageCode":
        """Get LanguageCode from ISO 639-2 code (e.g., 'eng', 'ger')."""
        if not code:
            return LanguageCode.NONE
        code = code.lower().strip()
        for lang in LanguageCode:
    ...
    @staticmethod
    def from_name(name: str | None) -> "LanguageCode":
        """Get LanguageCode from language name (English or native)."""
        if not name:
            return LanguageCode.NONE
        name = name.lower().strip()
        for lang in LanguageCode:
            if lang.name_en and lang.name_en.lower() == name:
                return lang
            if lang.name_native and lang.name_native.lower() == name:
                return lang
        return LanguageCode.NONE

    @staticmethod
    def from_string(value: str | None) -> "LanguageCode":
        """Flexible parsing - matches ISO codes, English name, or native name."""
        if not value:
            return LanguageCode.NONE
    ...
    @staticmethod
    def is_valid_language(language: str | None) -> bool:
        """Check if a string represents a valid language."""
        return LanguageCode.from_string(language) is not LanguageCode.NONE

    def to_iso_639_1(self) -> str | None:
        """Get ISO 639-1 code (e.g., 'en')."""
        return self.iso_639_1

    def to_iso_639_2_t(self) -> str | None:
        """Get ISO 639-2/T code (e.g., 'eng')."""
        return self.iso_639_2_t

    def to_iso_639_2_b(self) -> str | None:
        """Get ISO 639-2/B code (e.g., 'ger' for German)."""
        return self.iso_639_2_b

    def to_name(self, in_english: bool = True) -> str | None:
        """Get language name."""
    ...
    def __eq__(self, other: object) -> bool:
        """Compare LanguageCode to another object."""
        if other is None:
