...
`, {
  ...
  // skipIf: per-specialist cheap gate. CONFIG.skipIf = { name: 'shell-cmd' };
  // exit 0 = skip this round. One gate-agent runs all checks; rotation
  // advances past skipped names so the round isn't wasted (F47).
  let skip = []
  if (CONFIG.skipIf && Object.keys(CONFIG.skipIf).length) {
    const checks = Object.entries(CONFIG.skipIf)
      .map(([n, c]) => `(${c}) >/dev/null 2>&1 && echo "${n}"`).join('\n')
    const g = await agent(`
Gate check (cwd=_base, fresh origin/main). Run:
\`\`\`sh
${checks}
\`\`\`
Return \`skip\` = the names that printed (those whose condition held).`, {
      label: `gate-r${round}`, phase: 'Work',
      schema: { type: 'object', properties: { skip: { type: 'array', items: { type: 'string' } } }, required: ['skip'] },
    })
    skip = g?.skip ?? []
    if (skip.length) log(`gate: skip [${skip.join(', ')}]`)
  }
  let specName = CONFIG.forceSpecialist?.(triage)
  if (!specName) {
    const rot = SPECIALIST_NAMES.filter(n => !skip.includes(n))
    specName = rot.length ? rot[(round - 1) % rot.length] : SPECIALIST_NAMES[(round - 1) % SPECIALIST_NAMES.length]
  }
  const specSchema = {
    type: 'object',
    properties: {
  ...
  // Null-ify a thunk's failure so one transient agent error (e.g. an
  // Anthropic server-side rate-limit that makes a schema agent "complete
  // without calling StructuredOutput") degrades that branch to "nothing this
  // round" — which ok()/`if (impl)`/.filter(Boolean) below already tolerate —
  // instead of aborting a multi-hour run. parallel() null-ifies ordinary
  // thunk throws, but the StructuredOutput-miss is a fatal class that escapes
  // it (that's what killed the 2026-06-01 run), so guard the schema agents.
  const guard = (thunk) => () => Promise.resolve().then(thunk).catch(() => null)
  const [specOut, archOut, impls] = await parallel([
    guard(specTask),
    ...(runArch ? [guard(archTask)] : [() => null]),
    () => parallel(picks.map(p => guard(() => implStage(p)))),
  ])
